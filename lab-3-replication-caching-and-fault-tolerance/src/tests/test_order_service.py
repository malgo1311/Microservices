...
# FUNCTION TO CALL ORDER QUERY SERVICE
def call_order_query_service(order_number):
    ...
    print(f"QUERY - {order_number} - {code} - {response}")
    return code
...
class TestService(unittest.TestCase):
    ...
    # TEST QUERY REQUEST
    def test_order_query_function(self):

        print("\n\nTesting ORDER Query requests\n")
        
        # Case 1: TEST CASE WITH CORRECT ORDER NUMBER
        order_number = 1
        self.assertTrue( call_order_query_service(order_number) == 200, "Query request : Failed case 1")

        # Case 2: TEST CASE WITH INCORRECT ORDER NUMBER
        order_number = "abc"
        self.assertTrue( call_order_query_service(order_number) == 400, "Query request : Failed case 2")

        # Case 3: TEST CASE WITH CORRECT Query INFORMATION BUT WITH HIGH BUY VOLUME
        order_number = 99999999999999999999999999999999
        self.assertTrue( call_order_query_service(order_number) == 400, "Query request : Failed case 3")
