...
class RWLock:
    ...
    def acquire_read(self):
        with self.lock:
            if self.DEBUG:
                ...
                assert me not in self.active_readers, logger.debug('This thread has already acquired read access and this lock isn\'t reader-reentrant!')
                assert me != self.active_writer, logger.debug('This thread already has write access, release that before acquiring read access!')
    ...
    def release_read(self):
        with self.lock:
            if self.DEBUG:
                me = threading.currentThread()
                assert me in self.active_readers, logger.debug('Trying to release read access when it hasn\'t been acquired by this thread!')
    ...
    def acquire_write(self):
        with self.lock:
            if self.DEBUG:
                me = threading.currentThread()
                assert me not in self.active_readers, logger.debug('This thread already has read access - release that before acquiring write access!')
                assert me != self.active_writer, logger.debug('This thread already has write access and this lock isn\'t writer-reentrant!')
            self.writer_count += 1
    ...
    def release_write(self):
        ...
        with self.lock:
            if self.DEBUG:
                me = threading.currentThread()
                assert me == self.active_writer, logger.debug('Trying to release write access when it hasn\'t been acquired by this thread!')
                self.active_writer = None
                self.active_writer_lock.release()
